...
| **`tickImages`** (optional) | <pre>TickImage[]</pre>              | An array of objects that allows you to provide custom images for the axis ticks. Each object can contain an `image` URL or an `SkImage` object, along with optional width and height properties. This allows you to customize the appearance of the axis ticks with images. You'll want to make use of other props like `domain                                             ` and `tickValues` and `tickCount` in order to get this exactly how you want. Also note that by passing in a `image` vs `SkImage` will have different loading experiences, the former could potentially be a remote URL which will take time to load and the latter is more immediate. |
