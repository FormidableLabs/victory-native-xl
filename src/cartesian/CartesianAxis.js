...
export function useChartAxis({ yKeys, axisOptions, onScaleChange, xAxis, yAxis, frame, xScale, primaryYScale, chartBounds, yAxes, isNumericalData, _tData, hasMeasuredLayoutSize, scrollX, ignoreClip, }) {
    ...
    const YAxisComponents = hasMeasuredLayoutSize && (axisOptions || yAxes)
        ...
        : null;
    const XAxisComponents = hasMeasuredLayoutSize && (axisOptions || xAxis) ? (React.createElement(XAxis, { ...normalizedAxisProps.xAxis, scrollX: scrollX, xScale: xScale, yScale: zoomY.rescaleY(primaryYScale), ix: _tData.ix, isNumericalData: isNumericalData, chartBounds: chartBounds, zoom: zoomX, ignoreClip: ignoreClip })) : null;
