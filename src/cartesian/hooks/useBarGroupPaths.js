...
export const useBarGroupPaths = (points, chartBounds, betweenGroupPadding = 0, withinGroupPadding = 0, roundedCorners, customBarWidth, barCount) => {
    ...
    const paths = React.useMemo(() => {
        return points.map((pointSet, i) => {
            ...
            pointSet.forEach(({ x, y, yValue }) => {
                ...
                if (roundedCorners) {
                    const nonUniformRoundedRect = createRoundedRectPath(x + offset, y, barWidth, barHeight, roundedCorners, Number(yValue));
