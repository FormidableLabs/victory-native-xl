...
export const useStackedAreaPaths = ({ pointsArray, colors, y0, curveType = "linear", areaOptions = () => ({}), }) => {
    const paths = React.useMemo(() => {
        ...
        return pointsArray.map((points, layerIndex) => {
            ...
            groups.forEach((group) => {
                ...
                const stitchedData = stitchDataArray(group).map(([x, y], i) => {
                    const offset = offsets[layerIndex]?.[i] ?? 0;
                    const newY = y - offset;
                    return [x, newY];
                });
                lowestPointOfLayer = Math.max(...(offsets[layerIndex] ?? []).map((num) => y0 - num));
                ...
                if (svgPath) {
                    path.addPath(Skia.Path.MakeFromSVGString(svgPath) ?? Skia.Path.Make());
                }
            });
