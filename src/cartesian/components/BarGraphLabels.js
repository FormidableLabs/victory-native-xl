...
export const BarGraphLabels = ({ points, chartBounds, barWidth = 0, options, }) => {
    ...
    return points.map(({ x, y = 0, yValue }) => {
        ...
        return (React.createElement(Text, { key: `${xOffset}-${yOffset}-${yText}`, x: xOffset, y: yOffset, text: yText, font: font, color: color }));
    });
};
