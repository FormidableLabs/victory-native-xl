...
export const XAxis = ({ xScale: xScaleProp, yScale, axisSide = "bottom", yAxisSide = "left", labelPosition = "outset", labelRotate, tickCount = DEFAULT_TICK_COUNT, tickValues, labelOffset = 2, labelColor = "#000000", lineWidth = StyleSheet.hairlineWidth, lineColor = "hsla(0, 0%, 0%, 0.25)", font, formatXLabel = (label) => String(label), ix = [], isNumericalData, linePathEffect, chartBounds, enableRescaling, zoom, }) => {
    ...
    const xAxisNodes = xTicksNormalized.map((tick) => {
        ...
        return (React.createElement(React.Fragment, { key: `x-tick-${tick}` },
            ...
            font && labelWidth && canFitLabelContent ? (React.createElement(Group, { transform: [{ translateY: rotateOffset }] },
