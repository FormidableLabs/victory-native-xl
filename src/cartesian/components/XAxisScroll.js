...
export const XAxis = ({ xScale: xScaleProp, ignoreClip, yScale, axisSide = "bottom", yAxisSide = "left", labelPosition = "outset", labelRotate, tickCount = DEFAULT_TICK_COUNT, tickValues, labelOffset = 2, labelColor = "#000000", lineWidth = StyleSheet.hairlineWidth, lineColor = "hsla(0, 0%, 0%, 0.25)", font, formatXLabel = (label) => String(label), ix = [], isNumericalData, linePathEffect, chartBounds, enableRescaling, zoom, scrollX, }) => {
    ...
    const xAxisNodes = xTicksNormalized.map((tick, index) => {
        ...
        return (React.createElement(React.Fragment, { key: `x-tick-${tick}` },
            lineWidth > 0 ? (React.createElement(Group, { transform: transformX, clip: ignoreClip ? boundsToClip(chartBounds) : undefined },
                React.createElement(Line, { p1: p1, p2: p2, color: lineColor, strokeWidth: lineWidth }, linePathEffect ? linePathEffect : null))) : null,
            font && labelWidth && canFitLabelContent ? (React.createElement(Group, { transform: transformX },
                React.createElement(Text, { transform: [
                        {
                            translateX: index === 0 ? 10 : 0,
                            rotate: (Math.PI / 180) * (labelRotate ?? 0),
                        },
                    ], origin: origin, color: labelColor, text: contentX, font: font, y: labelY, x: labelX }))) : null,
