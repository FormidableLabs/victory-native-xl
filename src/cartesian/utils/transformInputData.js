 ...
 */
export const transformInputData = ({ data: _data, xKey, yKeys, outputWindow, domain, domainPadding, xAxis, yAxes, viewport, labelRotate, }) => {
    const data = [..._data];
    ...
    const yAxesTransformed = (yAxes ?? [{}])?.map((yAxis) => {
        ...
        const yScaleRange = (() => {
            const xTickCount = (typeof yAxis?.tickCount === "number"
                ? yAxis?.tickCount
                : xAxis?.tickCount) ?? 0;
        ...
        const yScale = makeScale({
            ...
            padStart: typeof domainPadding === "number" ? domainPadding : domainPadding?.top,
        });
        ...
        const maxYLabel = Math.max(...yTicksNormalized.map((yTick) => yAxis?.font
            ?.getGlyphWidths?.(yAxis.font.getGlyphIDs(yAxis?.formatYLabel?.(yTick) || String(yTick)))
    ...
    const ixMin = asNumber(domain?.x?.[0] ?? tickDomainsX?.[0] ?? ixNum.at(0)), ixMax = asNumber(domain?.x?.[1] ?? tickDomainsX?.[1] ?? ixNum.at(-1));
    ...
    const xScale = makeScale({
        ...
        padStart: typeof domainPadding === "number" ? domainPadding : domainPadding?.left,
        padEnd: typeof domainPadding === "number" ? domainPadding : domainPadding?.right,
    });
    ...
    if (labelRotate) {
        const maxXLabel = Math.max(...xTicksNormalized.map((xTick) => xAxis?.font
            ?.getGlyphWidths?.(xAxis.font.getGlyphIDs(xAxis?.formatXLabel?.(xTick) || String(xTick)))
            .reduce((sum, value) => sum + value, 0) ?? 0));
        ...
        const rotateLabelOffset = Math.abs(maxXLabel * getOffsetFromAngle(labelRotate));
        const yScaleRange0 = yAxesTransformed[0]?.yScale.range().at(0);
        const yScaleRange1 = yAxesTransformed[0]?.yScale.range().at(-1);
        // bottom, outset
