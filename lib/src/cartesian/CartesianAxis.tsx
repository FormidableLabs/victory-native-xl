...
export function useChartAxis({
  ...
  const xScaleRef = React.useRef<ScaleLinear<number, number> | undefined>(undefined)
  const yScaleRef = React.useRef<ScaleLinear<number, number> | undefined>(undefined)
      ...
      ? normalizedAxisProps.yAxes?.map((axis, index) => {
          ...
          return (
            <YAxis
              ...
              yScale={rescaled}
              yTicksNormalized={index > 0 && !axis.tickValues ? normalizeYAxisTicks(primaryTicksRescaled, primaryRescaled, rescaled) : rescaledTicks}
